...
import type { Goal, TrainingType, UserProfile } from '../types'
import { allEquipment } from '../data/exercises'
...
const goals:{id:Goal;name:string;desc:string}[]=[
  ...
  {id:'recomp',name:'Giảm mỡ + tăng cơ',desc:'Progressive overload với khối lượng trung bình.'},
...
export default function Onboarding({onDone,onDemo}:{onDone:(p:UserProfile)=>void;onDemo:()=>void}){
  ...
  const available=useMemo(()=>allEquipment.filter(e=>type==='gym'||!['Barbell','Squat Rack','Cable Machine','Lat Pulldown','Leg Press','Leg Extension','Leg Curl','Chest Press','Shoulder Press Machine','Pec Deck','Row Machine','Smith Machine','Hack Squat','Hip Abduction','Hip Adduction','Calf Machine','Seated Calf Machine','Back Extension Bench','Cardio Machine'].includes(e)),[type])
  ...
  const finish=()=>{
    ...
    onDone({id:uid(),name:name.trim()||'Bạn',gender,age,heightCm:height,weightKg:weight,targetWeightKg:target,trainingType:type,goal,experience:exp,daysPerWeek:days,trainingDays:selectedDays,sessionMinutes:minutes,activityLevel:'medium',equipment,benchmarks:{pushup:bench.pushup,pullup:bench.pullup,dip:bench.dip,bodyweight_squat:bench.squat,plank:bench.plank},injuries,unit:'kg',theme:'light',createdAt:new Date().toISOString(),dumbbell:type==='home'?db:undefined})
  }
  return <div className="onboarding-shell"><div className="brand"><div className="brand-mark">SW</div><div><b>Smart Workout Coach</b><span>HLV tự động · dữ liệu lưu trên thiết bị</span></div></div>
    <div className="onboarding card">
      ...
      {step===1&&<><p className="eyebrow">BƯỚC 1/6</p><h1>Bạn tập ở đâu?</h1><p className="muted">App chỉ dùng những bài phù hợp với dụng cụ bạn có.</p><div className="choice-grid">
        ...
        <button className={type==='home'?'choice selected':'choice'} onClick={()=>chooseType('home')}><b>Tạ đơn tại nhà</b><span>Dumbbell + xà đơn/xà kép, ít dụng cụ</span></button>
        <button className={type==='gym'?'choice selected':'choice'} onClick={()=>chooseType('gym')}><b>Phòng Gym</b><span>Tạ đòn, máy, cáp, dumbbell</span></button>
      </div></>}
      {step===2&&<><p className="eyebrow">BƯỚC 2/6</p><h1>Mục tiêu chính</h1><div className="choice-grid compact">{goals.filter(g=>g.id!=='skill'||type!=='gym').map(g=><button key={g.id} className={goal===g.id?'choice selected':'choice'} onClick={()=>setGoal(g.id)}><b>{g.name}</b><span>{g.desc}</span></button>)}</div>{goal==='definition'&&<div className="info">Độ nét cơ phụ thuộc nhiều vào tỷ lệ mỡ cơ thể. App sẽ ưu tiên giữ cơ và sức mạnh, không tạo “bài cắt nét” giả khoa học.</div>}</>}
      ...
      {step===5&&<><p className="eyebrow">BƯỚC 5/6</p><h1>Khả năng hiện tại</h1><p className="muted">Không biết thì để 0. Hệ thống sẽ học từ các buổi sau.</p><div className="form-grid"><label>Push-up tối đa<input type="number" min="0" value={bench.pushup} onChange={e=>setBench({...bench,pushup:+e.target.value})}/></label><label>Pull-up tối đa<input type="number" min="0" value={bench.pullup} onChange={e=>setBench({...bench,pullup:+e.target.value})}/></label><label>Dip tối đa<input type="number" min="0" value={bench.dip} onChange={e=>setBench({...bench,dip:+e.target.value})}/></label><label>Bodyweight Squat<input type="number" min="0" value={bench.squat} onChange={e=>setBench({...bench,squat:+e.target.value})}/></label><label>Plank (giây)<input type="number" min="0" value={bench.plank} onChange={e=>setBench({...bench,plank:+e.target.value})}/></label></div><label>Dụng cụ đang có</label><div className="chips wrap">{available.map(e=><button key={e} className={equipment.includes(e)?'chip active':'chip'} onClick={()=>toggle(e,equipment,setEquipment)}>{e}</button>)}</div>{type==='home'&&<div className="subcard"><b>Thông số Dumbbell</b><div className="form-grid"><label>Mức nhẹ nhất (kg)<input type="number" value={db.minKg} onChange={e=>setDb({...db,minKg:+e.target.value})}/></label><label>Mức nặng nhất (kg)<input type="number" value={db.maxKg} onChange={e=>setDb({...db,maxKg:+e.target.value})}/></label><label>Bước tăng (kg)<select value={db.stepKg} onChange={e=>setDb({...db,stepKg:+e.target.value})}>{[0.5,1,2,2.5,5].map(x=><option key={x}>{x}</option>)}</select></label></div></div>}</>}
