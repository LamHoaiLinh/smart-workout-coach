...
export default function Today({profile,day,sessions,activities,activeSession,onStart,onResume,onStartCardio}:{profile:UserProfile;day:ProgramDay;sessions:WorkoutSession[];activities:ActivitySession[];activeSession?:WorkoutSession;onStart:(r:Readiness)=>void;onResume:()=>void;onStartCardio:(p:CardioPlan)=>void}){
  ...
  return <main className="page"><header><p className="eyebrow">{dateLabel().toUpperCase()}</p><h1>Hôm nay</h1><p className="muted">{profile.name}, đây là phần tập hôm nay.</p></header>
    ...
    <section className="hero-card"><div><span className="pill">{focusLabel[day.focus]??day.focus}</span><h2>{day.title}</h2><p>{r.minutes} phút dự kiến · {day.exercises.length} bài{day.cardio?' · có thêm vận động':''}</p></div><div className="hero-score">{weekCount}<small>buổi / 7 ngày</small></div></section>
    ...
    {day.cardio&&<section className="card cardio-today"><div className="section-title"><div><p className="eyebrow">VẬN ĐỘNG THÊM</p><h3>{day.cardio.title}</h3></div><span className="cardio-duration">{day.cardio.minutes} phút</span></div><p className="muted">{day.cardio.note}</p><button className="btn secondary full" onClick={()=>onStartCardio(day.cardio)}>Bắt đầu {activityName({id:'x',mode:day.cardio.mode,startedAt:'',completedAt:'',durationSeconds:0})}</button></section>}
