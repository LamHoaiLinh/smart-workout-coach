...
export default function Workout({session,profile,history,onChange,onFinish,onCancel}:Props){
  ...
  const [overall,setOverall]=useState(3); const [fatigue,setFatigue]=useState(3); const [pain,setPain]=useState(false); const [painArea,setPainArea]=useState('')
  const item=session.exercises[exerciseIndex]
  const ex=exerciseById.get(item.exerciseId)
  const activeSet=Math.max(0,item.sets.findIndex(s=>!s.completed))
  const currentSet=item.sets[Math.min(activeSet,item.sets.length-1)]
  const [reps,setReps]=useState(currentSet?.reps??item.planned.minReps??0)
  const [seconds,setSeconds]=useState(currentSet?.seconds??item.planned.seconds??0)
  const [weight,setWeight]=useState(currentSet?.weightKg??item.planned.weightKg??0)
  ...
  const swaps=suggestSwap(item.exerciseId,profile,used)
  const updateItem=(next:any)=>{const arr=[...session.exercises];arr[exerciseIndex]=next;onChange({...session,exercises:arr})}
  const completeSet=()=>{
    const idx=item.sets.findIndex(s=>!s.completed);if(idx<0)return
    const sets=item.sets.map((s,i)=>i===idx?{...s,reps:item.planned.seconds?undefined:reps,seconds:item.planned.seconds?seconds:undefined,weightKg:item.planned.weightKg!==undefined?weight:undefined,completed:true}:s)
    updateItem({...item,sets});setRest(item.planned.restSeconds)
  }
  ...
  const swap=(id:string)=>{const e=exerciseById.get(id);if(!e)return;const p={...item.planned,exerciseId:e.id,name:e.nameEnglish,minReps:e.minReps,maxReps:e.maxReps,seconds:e.holdSeconds,restSeconds:e.recommendedRest,weightKg:e.weighted?item.planned.weightKg:undefined,selectionReason:'Bạn đã chủ động đổi bài. App giữ cùng kiểu vận động, dụng cụ phù hợp và độ khó gần tương đương.',progressionReason:undefined};updateItem({exerciseId:e.id,name:e.nameEnglish,planned:p,sets:Array.from({length:p.sets},()=>({weightKg:p.weightKg,completed:false}))});setShowSwap(false)}
  const allDone=item.skipped||item.sets.every(s=>s.completed)
  const feedbackItems= simple
    ?([['too_easy','Quá nhẹ'],['good','Vừa'],['hard','Khó'],['near_limit','Gần hết sức'],['failed','Không hoàn thành']] as [DifficultyFeedback,string][])
    :([['too_easy','Quá nhẹ · RIR 4+'],['good','Vừa · RIR 2–3'],['hard','Khó · RIR 1–2'],['near_limit','Gần hết sức · RIR 0–1'],['failed','Không hoàn thành · RIR 0']] as [DifficultyFeedback,string][])
  return <div className="workout-shell"><header className="workout-top"><button className="icon-btn" onClick={onCancel}>×</button><div><small>{session.title}</small><b>Bài {exerciseIndex+1}/{session.exercises.length}</b></div><span>{Math.round(session.exercises.filter(e=>e.skipped||e.sets.every(s=>s.completed)).length/session.exercises.length*100)}%</span></header>
    <main className="workout-main"><div className="progress-line"><i style={{width:`${(exerciseIndex+1)/session.exercises.length*100}%`}}/></div><section className="workout-title"><span className="pill">{ex?.primaryMuscles.join(' · ')}</span><h1>{item.name}</h1><p>{ex?.nameVietnamese}</p>{previous&&<div className="last-time">Lần trước: {previous.sets.filter(s=>s.completed).map(s=>s.reps??s.seconds).join(' / ')} {item.planned.seconds?'giây':'lần'}</div>}{item.planned.progressionReason&&<div className="progression-callout"><b>App đã điều chỉnh:</b> {item.planned.progressionReason}</div>}</section>
      <section className="set-card"><div className="set-head"><b>Hiệp {Math.min(activeSet+1,item.sets.length)} / {item.sets.length}</b><span>Mục tiêu {item.planned.seconds?`${item.planned.seconds} giây`:`${item.planned.minReps}–${item.planned.maxReps} lần`}</span></div>{!allDone&&<>{item.planned.weightKg!==undefined&&<Counter label="Mức tạ" value={weight} step={profile.trainingType==='home'?(profile.dumbbell?.stepKg??1):2.5} suffix="kg" onChange={setWeight}/>}<Counter label={item.planned.seconds?'Thời gian':'Số lần'} value={item.planned.seconds?seconds:reps} step={1} suffix={item.planned.seconds?'giây':'lần'} onChange={item.planned.seconds?setSeconds:setReps}/><button className="btn primary full xl" onClick={completeSet}>Hoàn thành hiệp</button></>}
        <div className="set-dots">{item.sets.map((s,i)=><span key={i} className={s.completed?'done':''}>{i+1}</span>)}</div></section>
      {allDone&&<section className="card"><h3>Bài này cảm giác thế nào?</h3>{simple&&<p className="muted">Chỉ cần chọn cảm giác gần nhất; app sẽ tự chuyển phản hồi này thành dữ liệu điều chỉnh.</p>}<div className="feedback-grid">{feedbackItems.map(([v,n])=><button className={item.feedback===v?'active':''} key={v} onClick={()=>feedback(v)}>{n}</button>)}</div><div className="actions"><button className="btn ghost" onClick={()=>setShowSwap(!showSwap)}>Đổi bài</button><button className="btn primary" onClick={goNext}>{exerciseIndex===session.exercises.length-1?'Kết thúc buổi':'Bài tiếp theo'}</button></div></section>}
...
function Counter({label,value,step,suffix,onChange}:{label:string;value:number;step:number;suffix:string;onChange:(v:number)=>void}){return <div className="counter"><span>{label}</span><div><button onClick={()=>onChange(Math.max(0,Math.round((value-step)*10)/10))}>−</button><b>{value}<small>{suffix}</small></b><button onClick={()=>onChange(Math.round((value+step)*10)/10)}>+</button></div></div>}
